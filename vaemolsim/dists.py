...
class ParamTransform(object):
  ...
  def __init__(self,
               dist_class=None,
               transform_fn=lambda x: tf.Identity(x)):
    """
    Inputs:
        dist_class - (tfp.distributions class, None) a class within tfp.distributions with the attribute parameter_properties
    ...
    if dist_class is not None:
      ...
      if dist_class.__name__ == 'VonMises':
        ...
        def _fn(x):
          return {'loc': props_dict['loc'].default_constraining_bijector_fn()(tf.math.atan2(x[..., 0], x[..., 1])),
                  'concentration': props_dict['concentration'].default_constraining_bijector_fn()(x[..., 2])}

      else:

        def _fn(x):
          return {k: props_dict[k].default_constraining_bijector_fn()(x[..., i])
...
class IndependentBlockwise(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    Inputs:
        ...
        param_nums - (int or list of ints, None) list of integers specifying the number of parameters to pass to each
                     ...
                     if left as None, the default, the number of parameters will be inferred from the classes in dist_classes
        param_transforms - (callable or list of callables, tf.Identity) should be a callable function or list of callable
                           functions with a single callable function for each dof that expects to transform param_nums[i] inputs
                           for the ith dof distribution; if left as None (the default), will infer transformations from dist_classes
                           using the parameter_properties attribute and wrapping with a callable ParamTransform class
    Outputs:
    ...
    if not isinstance(dist_classes, (list, tuple)):
      ...
      if len(dist_classes) != self.num_dofs:
        raise ValueError("If specifying a list of tfp.distributions objects, must be of same length as number of degrees of freedom (%i) but got %i."%(self.num_dofs, len(dist_classes)))
    ...
    if param_nums is None:
      ...
      if not isinstance(param_nums, (list, tuple)):
        ...
        if len(param_nums) != self.num_dofs:
          raise ValueError("If specifying a list of parameter numbers, must be of same length as number of degrees of freedom (%i) but got %i."%(self.num_dofs, len(param_nums)))
        self.param_nums = param_nums
    ...
    if param_transforms is None:
      ...
      if not isinstance(param_transforms, (list, tuple)):
        ...
        if len(param_transforms) != self.num_dofs:
          raise ValueError("If specifying a list of parameter transformations, must be of same length as number of degrees of freedom (%i) but got %i."%(self.num_dofs, len(param_transforms)))
  ...
  def get_config(self):
    ...
    config.update({"num_dofs": self.num_dofs,
...
class AutoregressiveBlockwise(IndependentBlockwise):
  ...
  def build(self, input_shape):
    ...
    if input_shape[-2:] != (self.num_dofs, max(self.param_nums)):
      raise ValueError("Last (assuming only non-batch) dimension is of size %s, but must match number of specified degrees of freedom, %s."%(str(input_shape[-2:]), str((self.num_dofs, max(self.param_nums)))))
  ...
  def call(self, inputs, conditional_input=None):
    ...
    return tfp.distributions.Autoregressive(_make_dist, sample0=tf.ones(tf.shape(inputs)[:-1]), num_steps=self.num_dofs)
  ...
  def get_config(self):
    ...
    config.update({"conditional": self.conditional,
                   "conditional_event_shape": self.conditional_event_shape,
                  })
    return config
...
class IndependentVonMises(tfp.layers.DistributionLambda):
  ...
  def __init__(self,
    ...
    super(IndependentVonMises, self).__init__(
        lambda t: IndependentVonMises.new(t, event_shape, validate_args),
    ...
    with tf.name_scope(name or 'IndependentVonMises'):
      ...
      return tfp.distributions.Independent(
          tfp.distributions.VonMises(
              ...
              concentration=tf.math.softplus(tf.reshape(scale_params, output_shape)),
...
class IndependentDeterministic(tfp.layers.DistributionLambda):
  ...
  def __init__(self,
    ...
    super(IndependentDeterministic, self).__init__(
        lambda t: IndependentDeterministic.new(t, event_shape, validate_args),
        convert_to_tensor_fn,
        **kwargs)
    ...
    with tf.name_scope(name or 'IndependentDeterministic'):
      ...
      return tfp.distributions.Independent(
          tfp.distributions.Deterministic(
              loc=tf.reshape(params, output_shape),
              validate_args=validate_args),
          reinterpreted_batch_ndims=tf.size(event_shape),
          validate_args=validate_args)
...
#It's complicated because of the need to update the FG coordinates supplied to the distance mask and attention
#We want to create the distributions for each set of FG degrees of freedom within the local coordinate system of the CG site
#That makes the probabilistic modeling easier and more tightly coupled to the physics (can use internal DOFs, etc.)
#BUT, for the masking and geometric algebra attention, we need Cartesian coordinates in the global frame
...
#But then each time you call _make_dist_fn, you're having to recompute that mapping to global Cartesian coords for ALL previous
#Is it possible to just keep a tensor of FG Cartesian coords owned by the layer and have each _make_dist_fn slice over that?
...
#And the one shown is where you compute global coords for all previously decoded residues rather than stashing them and slicing
class JointDistribution(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    Inputs:
        ...
        coord_map_dict - a dictionary mapping names of residues/molecules to mappings from a CG site origin to rectangular atomic coords;
                         the callable for the mapping should take as arguments the local (internal) coordinates and the CG coordinate;
  ...
  def call(self, inputs, training=False):
    #Treat each input as coordinates of a CG site (could be just 3 rectangular coords, or backbone plus centroid, etc.)
    #Or at least it's much harder with proteins... probably actually just want CG coords but exclude some backbone atoms somehow
    #Just need to make sure the atomic coords you decode to are internal (BAT) coords for JUST sidechain atoms
    ...
    #For others, predict bonds, angles, dihedrals, etc. as needed with alpha carbon as root atom and ignoring beta carbon coords
    ...
    #Each of the entries along axis 1 are the fixed inputs to a function to produce a distribution, of shape (N_batch, N_descriptors)
    #That way they are of fixed size and should contain info about the nearest neighbor CG sites and their properties
    descriptors = cgschnet(inputs, self.res_names) #Or pass property list instead of names? Something like LJ params, charges, etc.
    #Because will have to pass atom properties, can have local CG attention network same as FG (just different atom types)
    #Could also make separate, but seems easier to just concatenate all coordinates and properties together
    #As an extra bonus, concatenation will let CG and FG coordinates attend to each other, which may improve performance

    #Some way to create random permutation/decoding order?  Probably do when encoding, actually, then can sort out after decoding.
    #Do in overall model!
    ...
    #Note that distribution creating callables in dist_dict should have mapping to convert descriptors to shape they need
    #So maybe should actually be MappingToDistribution objects
    dist_list.append(self.dist_dict[self.res_names[0]](descriptors[:, 0, :], atom_descriptors=None, training=training))
    ...
    for i in range(1, len(self.res_names)):
      ...
      def _make_dist_fn(*args):
        ...
        #Each item in list should be the residue/molecule local atomic coordinates with its CG site as the origin
        ...
        #Apply an schnet to the decoded coordinates (may just be way to collect nearby coordinates and convert to local coords)
        ...
        return self.dist_dict[self.res_names[i]](descriptors[:, i, :], atom_descriptors=atom_descriptors, training=training)

      dist_list.append(_make_dist_fn)

    return JointDistributionSequentialAutoBatched(dist_list, batch_ndims=1)
